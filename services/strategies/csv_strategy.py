...
class CsvSnapshotStrategy(SnapshotStrategy):
    """Snapshot strategy for CSV format."""
    ...
    def save_snapshot(
        ...
        """Save snapshot data as CSV files.
        ...
        try:
            ...
            if changes.get("added"):
                ...
                if self._save_csv_file(filename, df):
            ...
            # Save summary as JSON (CSV tidak cocok untuk nested summary)
            summary_filename = self._generate_filename(table_name, datasource_name, timestamp, "summary").replace('.csv', '.json')
            summary_data = {
            ...
            logger.error(f"Error saving CSV snapshot: {str(e)}")
            return {
                "status": "error",
                "format": "csv",
                "message": str(e)
    ...
    def _save_csv_file(self, filename: str, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            return self.storage_manager.store_snapshot(filename, data, content_type='text/csv')
